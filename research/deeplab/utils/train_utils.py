...
def add_sigmoid_cross_entropy_loss_for_each_scale(scales_to_logits,
  ...
  """
  if labels is None:
    raise ValueError('No label for sigmoid cross entropy loss.')

  for scale, logits in six.iteritems(scales_to_logits):
    loss_scope = None
    if scope:
      loss_scope = 'sigmoid_%s_%s' % (scope, scale)
    ...
    tensors = [not_ignore_mask for i in range(num_classes)]
    not_ignore_mask = tf.stack(tensors, axis=1)
