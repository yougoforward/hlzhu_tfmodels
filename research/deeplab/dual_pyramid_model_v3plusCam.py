      ...
      #       global_attention= tf.nn.sigmoid(image_feature, name=None)
            ...
            if decoder_use_separable_conv:
              ...
              decoder_features2 = split_separable_conv2d(
                ...
                # # If crop_size is None, we simply do global pooling.
                # image_feature1 = tf.reduce_mean(decoder_features_list1[0], axis=[1, 2])[:, tf.newaxis,
                #                  tf.newaxis]
                # image_feature1 = slim.conv2d(image_feature1, skip_depth, 1, activation_fn=tf.nn.relu,
                #                              normalizer_fn=slim.batch_norm,
                #                              scope=IMAGE_POOLING_SCOPE + str(i) + '_fuse1_conv1x1_0')
                # # image_feature1 = slim.conv2d(image_feature1, skip_depth, 1, activation_fn=None,
                # #                              normalizer_fn=None,
                # #                              scope=IMAGE_POOLING_SCOPE + str(i) + '_fuse1_conv1x1_1')
                # # global_attention1 = tf.nn.sigmoid(image_feature1, name=None)
                # skip_features1 = tf.multiply(decoder_features_list1[1], image_feature1, name=None)
                #
                ...
                #                              normalizer_fn=slim.batch_norm,
                #                              scope=IMAGE_POOLING_SCOPE + str(i) + '_fuse1_conv1x1_0')
                # # image_feature2 = slim.conv2d(image_feature2, skip_depth, 1, activation_fn=None,
                # #                              normalizer_fn=None,
                # #                              scope=IMAGE_POOLING_SCOPE + str(i) + '_fuse1_conv1x1_1')
                # # global_attention2 = tf.nn.sigmoid(image_feature2, name=None)
                # skip_features2 = tf.multiply(decoder_features_list2[1], image_feature2, name=None)
                #
...
def get_class_aware_attention_branch_logits(features,
  ...
  with slim.arg_scope(
    ...
    #       normalizer_fn=None,
    #       scope=scope+"class_aware_attention")
    # class_aware_attention = f2
    # # smin = tf.reduce_min(context_sensitive_logits, axis=3, keepdims=True, name=None)
    # # submin = tf.subtract(context_sensitive_logits, smin, name=None)
    # s1 = tf.nn.sigmoid(class_aware_attention, name=None)
    # attentioned_score = tf.multiply(context_sensitive_logits, s1, name=None)
    # # addmin = tf.add(attentioned_score, smin, name=None)
    #
    # return [attentioned_score]
    return [context_sensitive_logits]
